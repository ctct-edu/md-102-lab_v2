...
8. Stay signed in プロンプトで、**はい** を選択します。

9. **アカウントのセキュリティ保護** ダイアログで、**次へ** を選択します。

10. **Install Microsoft Authenticator** ダイアログで、**次へ** を選択します。

11. **アプリでアカウントを設定する** ページで、**次へ** を選択します。
...
18. Uncheck **パスワードの自動生成** を選択します。

19. **パスワード** の横に **Pa55-w.rd!** を選択します。
...
23. **ユーザーの種類** の横で **メンバー** が選択されていることを確認します。

    > 注: **メンバー** は既定のユーザー タイプであり、組織内のほとんどのユーザーに使用されます。

24. **役職** の横に **HR Rep** と入力します。

25. **部署** の横に **HR** と入力します。

26. **利用場所** の横で **米国** を選択します。
...
33. Uncheck **パスワードの自動生成** を選択します。

34. **パスワード** の横に **Pa55-w.rd!** を選択します。
...
38. **ユーザーの種類** の横で **メンバー** が選択されていることを確認します。

39. **役職** の横に **Helpdesk Manager** と入力します。

40. **部署** の横に **Operations** と入力します。

41. **利用場所** の横で **米国** を選択します。
...
4. **PowerShell-7.5.4-win-x64.msi** をダブルクリックして、セットアップ ウィザードを起動します。  。
   ...
   - **次へ** を選択します。  
   - **Destination Folder** は変更せず、**次へ** を選択します。  
   - **Optional Actions** は変更せず、**次へ** を選択します。  
   - *Use Microsoft Update to help keep your computer secure and up to date* のチェック ボックスをオフのままにし、**次へ** を選択します。  
   - **Install** を選択します。  
...
9. **Sign in** ダイアログで管理者パスワードを使用して **`admin@yourtenant.onmicrosoft.com`** としてサインインし、**サインイン** を選択します。
...
| -------------- | ---------------------------------------- | --------------------------- |
| Allan Deyoung  | Manage the tenant                        | グローバル管理者            |
| Edmund Reeve   | Manage users, group, and password resets | ユーザー管理者              |
| Miranda Snider | Manage password resets                   | ヘルプデスク管理者          |
...
3. 検索ボックスを使用して検索します: **グローバル管理者** を選択します。

4. **グローバル管理者**を選択します。チェック ボックスではなく名前を選択します。

5. **グローバル管理者** で **割り当ての追加** を選択します。

6. **メンバーの選択**で **メンバーが選択されていません**を選択し、検索して**Allan Deyoung** を選択します。

7. **選択**、**次へ**、**割り当て** の順に選択します。

8. ナビゲーション階層リンクで、 **ロールと管理者 | すべてのロール** を選択します。

9. 検索ボックスを使用して検索します: **ユーザー管理者** を選択します。

10. **ユーザー管理者**を選択します。チェック ボックスではなく名前を選択します。

11. **ユーザー管理者** で **割り当ての追加** を選択します。

12. **メンバーの選択**で **メンバーが選択されていません**を選択し、検索して**Edmund Reeve** を選択します。

13. **選択**、**次へ**、**割り当て** の順に選択します。

14. ナビゲーション階層リンクで、 **ロールと管理者 | すべてのロール** を選択します。

15. 検索ボックスを使用して検索します: **ヘルプデスク管理者** を選択します。

16. **ヘルプデスク管理者**を選択します。チェック ボックスではなく名前を選択します。

17. **ヘルプデスク管理者** で **割り当ての追加** を選択します。

18. **メンバーの選択**で **メンバーが選択されていません**を選択し、検索して**Miranda Snider** を選択します。
...
4. Under Members, **メンバーが選択されていません** を選択します。
...
4. **会社のブランド** で、 **既定のサインイン エクスペリエンス**で、 **カスタマイズ** を選択します。
...
9. Cody Godinez の プロファイル ページで、中央のナビゲーション メニューから **ライセンス** を選択します。
   ...
   > 次の点を確認します: Cody does not have any current license assignments. And that licensing must now be performed in the Microsoft 365 管理センター.
...
17. Microsoft 365 管理センター で、 ナビゲーション ペインで、 **課金** > **ライセンス** を選択します。

18. **サブスクリプション** の一覧で、**Enterprise Mobility + Security E5** を選択します。

19. **グループ** タブを選択し、**ライセンスの割り当て** を選択します。

20. Navigate into the **グループ名を入力** textbox, and 次を選択します: **Contoso_Managers** group。

21. **ライセンスの割り当て** を選択します。

22. **Contoso_Managers にライセンスを割り当てました** ペインの右上隅にある **X** を選択して閉じます。

23. upper-left corner of the **Enterprise Mobility + Security E5** page で、 次を選択します: **ライセンスに戻る** link。

24. **サブスクリプション** の一覧で、**Office 365 E5 (no Teams)** を選択します。

25. **グループ** タブを選択し、**ライセンスの割り当て** を選択します。

26. Navigate into the **グループ名を入力** textbox, and 次を選択します: **Contoso_Managers** group。

27. **ライセンスの割り当て** を選択します。

28. **Contoso_Managers にライセンスを割り当てました** ペインの右上隅にある **X** を選択して閉じます。

29. Microsoft 365 管理センター で、 ナビゲーション ペインで、 **課金** > **ライセンス** を選択します。
