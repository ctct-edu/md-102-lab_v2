...
5. **アプリケーションの登録** ページで、次を入力します。
   ...
   - **サポートされているアカウントの種類:** シングルテナントのみ - Contoso (既定値。これは "この組織ディレクトリ内のアカウントのみ" のモダンなラベルです)
...
### 演習 3: （オプション）Pharmacy Helpdesk 委任ロールのエンドツーエンドでの割り当てと検証
...
3. アクティビティでログをフィルター処理します。例えば以下等、できる限りで実施してください。
...
#### タスク 3: 診断設定の理解 (概念のみの紹介です。実施はしません。)
...
#### タスク 4: （オプション）以前のラボの条件付きアクセス、コンプライアンス、RBAC 操作のトレース
...
#### タスク 1: （オプション）デバイス コンプライアンス レポートの生成
