...
2. 一覧から **Microsoft Defender for Endpoint セキュリティ ベースライン** を選択します。
...
7. **Configration settings** ページで、**検索** ボックスを使用して、以下の各設定を見つけて構成します。

8. **ドメイン ネットワーク ファイアウォール** を構成します。
   
   - **ドメイン ネットワーク ファイアウォールを有効にする:** True（既定値）
   - **ステルス モードを無効にする**: False（既定値）
...
10. **パブリック ネットワーク ファイアウォール** を構成します。
    - **パブリック ネットワーク ファイアウォールを有効にする:** True（既定値）
    - **ステルス モードを無効にする:** False（既定値）
    - **パブリック プロファイルの既定の受信アクション:** ブロック（既定値）
    - **成功した接続のログを有効にする:** 成功した接続のログ記録を有効にする
    - **破棄されたパケットのログを有効にする:** ドロップされたパケットのログ記録を有効にする
...
12. **Scope tags** ページで、**Pharmacy** を追加して、**次へ** を選択します。

13. **Assignments** ページで、**dyn-Windows-Devices** に割り当てます。
...
### タスク 2: （オプション）BitLocker 暗号化の状態を監視する
...
### タスク 3: （オプション）BitLocker 回復キーを取得する
...
## 演習 4: （オプション）Microsoft Tunnel Gateway を展開する
...
1. **SEA-DEV1** で、**Microsoft Intune 管理センター** の **テナント管理** > **Microsoft Tunnel ゲートウェイ** に移動します。
...
### タスク 2: （オプション）条件付きアクセス ポリシーをレポート専用からオンに切り替える
